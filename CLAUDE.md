...
**Offline storage management** (`storage_manifest` IndexedDB store, schema v5+): every captured/downloaded media file is indexed at write time (`mediaStore.ts`, `configMediaCache.ts`) with its **actual** byte size (`src/utils/byteSize.ts` — never a base64 string length). `projectDiscardService.ts` is the only supported way to remove a project's offline copy from a device — it is **local-only** (never calls the server, never `purgeProject()`) and blocks on any `dirty` record, `pending_actions`, or `dropped_actions` row. See `docs/OFFLINE_STORAGE_MANAGEMENT.md`.
