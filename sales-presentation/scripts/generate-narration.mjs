...
const scenes = [
  {
    id: "01",
    text: "Welcome to Commtrac. The field operations platform built for telecom and utility teams who need clarity, control, and confidence—from the office to the job site.",
  },
  {
    id: "02",
    text: "Field projects move fast. Paper checklists get lost. Photos sit on personal phones. Issues fall through the cracks. Commtrac brings every project, asset, and action into one trusted system.",
  },
  {
    id: "03",
  ...
  {
    id: "10",
    text: "Commtrac is built for real field conditions. On mobile, data syncs while connected—then technicians keep working offline. Photos, issues, and workflow progress queue safely until reconnect.",
  },
  {
    id: "11",
  ...
  {
    id: "12",
    text: "Commtrac turns fragmented field operations into a connected, accountable workflow—from first install to final sign-off. Ready to see it on your projects? Let's talk.",
  },
];
...
for (const scene of scenes) {
  ...
  if (existsSync(mp3)) {
    console.log(`skip scene-${scene.id}.mp3 (exists)`);
    continue;
  }
  const webm = join(outDir, `scene-${scene.id}.webm`);
  console.log(`generating scene-${scene.id}.mp3 ...`);
  const r = spawnSync(
    edgeTts,
    ["--voice", voice, "--rate=-5%", "--text", scene.text, "--write-media", webm],
    { stdio: "inherit" }
  );
