...
import { projectAssetService, type OpenAssetItem, type WorkloadSummaryItem, type ProjectAssetSummaryItem } from "../../services/projectAssetService";
...
const Dashboard = () => {
  ...
  const filteredProjects = useMemo(() => {
    ...
    const officeFiltered = (activeOffice === "All" || isAdmin)
      ? projects
      : projects.filter((p) => {
          if (p.officeId && officeIdsForRegion) return officeIdsForRegion.has(p.officeId);
  ...
  }, [activeOffice, projects, countryForOffice, officeIdsForRegion, isMyWork, user?.role, user?.fullName, myDashProjectIds]);
          ...
          <Box className="glass-card" sx={{ p: 3 }}>
            ...
            <Grid container spacing={2}>
              {(activeOffice === "All" ? availableCountries : [activeOffice]).map((region) => {
                ...
                // Also count assets from assetSummaryMap for projects in region
                const rProjectAssets = rp.reduce((sum, p) => sum + (assetSummaryMap.get(p.id) ?? 0), 0);
                const regionActiveInstalls = Math.max(rAssets, rProjectAssets);
                return (
                  <Grid key={region} item xs={12} md={4}>
                    <Box onClick={() => { updateActiveOffice(region); navigate("/projects"); }}
                      ...
                      <Typography variant="body2" color="text.secondary">{regionActiveInstalls} active installations</Typography>
                    </Box>
                  </Grid>
                );
              })}
