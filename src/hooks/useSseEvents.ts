...
export function useSseEvents() {
  ...
  useEffect(() => {
    ...
    const onAssetsUpdatedFromServer = (detail: Record<string, unknown>) => {
      // Asset list refresh is handled by page listeners; also refresh notification inbox
      // so PM-created assignments show in the bell badge without waiting for poll interval.
      ...
      prefetchRef.current = setTimeout(() => {
        ...
        if (projectId && userId) {
          ...
          // Fallback: assigned-scope bootstrap after upload drain (workflows + docs).
          scheduleBootstrapAfterUploadDrain("assigned", 0);
        }
