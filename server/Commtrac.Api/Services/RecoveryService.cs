...
LIMIT 100;";
    ...
    private async Task<SelectiveRestoreResultDto> RestoreAssetFromBackupAsync(string backupPath, string assetId, CancellationToken cancellationToken)
    {
        ...
        UpsertAsset(asset);
        await _db.SaveChangesAsync(cancellationToken);
        return new SelectiveRestoreResultDto("asset", asset.Id, asset.AssetTag, 1 + (string.IsNullOrWhiteSpace(note) ? 0 : 1), string.IsNullOrWhiteSpace(note) ? null : note);
    }
...
LIMIT 1;";
    ...
    private static string? NormalizeEntityType(string? entityType)
    {
        ...
        {
            null or "" or "all" => null,
            "project" or "asset" or "document" => type,
            _ => throw new InvalidOperationException("Unsupported entity type.")
        };
    }
