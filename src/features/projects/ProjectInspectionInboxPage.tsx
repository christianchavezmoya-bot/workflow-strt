...
export default function ProjectInspectionInboxPage({ projectId, assets, assetId, onChanged }: Props) {
  ...
  async function load() {
    ...
    setError(null);
    try {
      const data = await inspectionImportService.list({ projectId, assetId });
      setItems(data);
    } catch {
  ...
  async function handleAssign(item: InspectionImport) {
    ...
    try {
      await inspectionImportService.assign(item.id, { projectId, assetId: nextAssetId });
      await load();
      await onChanged?.();
    } catch {
                  ...
                  <Stack spacing={1} sx={{ minWidth: { md: 240 } }}>
                    <FormControl fullWidth size="small">
                      ...
                      <Select
                        ...
                        value={pendingAssignments[item.id] ?? item.projectAssetId ?? item.assetId ?? ""}
